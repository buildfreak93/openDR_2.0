...
def erode_thresh(image):
    ...
    #threshold the image
    ret,threshed=cv2.threshold(image,threshold_value,125,cv2.THRESH_BINARY)
...
def ellipse_fit(image,cont_img):
    ...
    imgcharan, contours, hierarchy = cv2.findContours(cont_img, cv2.RETR_EXTERNAL,
...
def extract_fundus(filename):
    print 'entered ext_fundus'
    test_img=cv2.imread(filename)
    #cv2.namedWindow('entered ext_fundus',cv2.WINDOW_AUTOSIZE)
    #cv2.imshow('entered ext_fundus',test_img)
    #cv2.waitKey(0)
    print 'cv2 read'
    print filename
    cv2.imwrite('read.jpg',test_img)
